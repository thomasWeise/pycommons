...
def test_examples_from_md() -> None:
    ...
    with pytest.raises(
            ValueError, match=f"No example found in {repr(base)[:-1]}.*"):
    ...
    with pytest.raises(
            ValueError, match="No end mark for start mark.*"):
    ...
    with pytest.raises(ValueError, match="No end mark for start mark.*"):
    ...
    with pytest.raises(
            ValueError, match="Did not find newline in stripped fragment.*"):
    ...
    with pytest.raises(ValueError, match="Error when compiling.*"):
        check_examples_in_md(base.resolve_inside("md_error_fragment_1.md"))

    with pytest.raises(ValueError, match="Error when compiling.*"):
        check_examples_in_md(base.resolve_inside("md_error_fragment_1.md"))

    with pytest.raises(ValueError, match="Error when executing.*"):
