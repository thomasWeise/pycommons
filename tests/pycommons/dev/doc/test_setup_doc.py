...
def test_setup_doc() -> None:
    ...
    with pytest.raises(
            TypeError, match=re_compile(
                r"descriptor '__len__' requires a 'str'.*")):
    ...
    with (pytest.raises(
            ValueError, match=re_compile(
                r"copyright_start_year=-1 is invalid, must.*")),
            temp_dir() as td):
    ...
    with (pytest.raises(
            ValueError, match=re_compile(
                f"copyright_start_year={current_year + 1}.*")),
            temp_dir() as td):
        setup_doc(td, prj_base, current_year + 1, None, None, None, None)

    with (pytest.raises(
            TypeError, match=re_compile(
                r"dependencies should be an instance of.*")),
            temp_dir() as td):
        setup_doc(td, prj_base, 2023, 1, None, None, None)  # type: ignore

    with (pytest.raises(
            TypeError, match=re_compile(
                r"descriptor 'strip' for 'str' objects doe.*")),
            temp_dir() as td):
        setup_doc(td, prj_base, 2023, (1, ), None, None, None)  # type: ignore
    ...
    with (pytest.raises(
            ValueError, match=re_compile(
                r"'yyyx-b' is not among the known.*")),
            temp_dir() as td):
